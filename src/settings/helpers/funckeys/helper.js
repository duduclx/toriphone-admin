...
const getLabel = (destination) => {
  // requirements
  const { groupsAll, conferencesAll, queuesAll } = useApis();
  ...
  */
  ...
  switch (destination.type) {
    ...
    case "queue":
      const queue = queuesAll.items.find((item) => item.id === destination.queue_id);
      return {
        ...destination,
    ...
    case "groupmember":
      const group = groupsAll.items.find((item) => item.id === destination.group_id);
      return {
        ...destination,
    ...
    case "conference":
      const conference = conferencesAll.items.find((item) => item.id === destination.conference_id);
      return {
        ...destination,
